...
<form name="ExportListForm" action="{'ezxmlexport/delete/'|ezurl( 'no', 'full' )}" method="post">
    ...
    <div class="context-attributes">
        ...
        {def $exportList = fetch( 'ezxmlexport', 'exports', hash( 'customer_id', $customer.id ) )}
        ...
        <table class="list" cellspacing="0">
            ...
            {foreach $exportList as $export sequence array( bglight, bgdark ) as $tr_class}
                ...
                {set $view_link  = concat( 'ezxmlexport/view/export/', $export.id)|ezurl('no', 'full')}
                {set $edit_link = concat( 'ezxmlexport/edit/(type)/export/(export)/', $export.id)|ezurl('no', 'full')}
                <tr class={$tr_class}>
                    <td>
    ...
    {* DESIGN: Control bar START *}<div class="box-bc"><div class="box-ml"><div class="box-mr"><div class="box-tc"><div class="box-bl"><div class="box-br">
        ...
        <div class="block">
            <input type="hidden" name="RedirectURI" value="{concat( 'ezxmlexport/view/customer/', $customer.id)}"/>
            <input class="button" type="submit" value="Remove selected" name="DeleteExportButton"/>
            <input class="button" type="button" value="{'Create an export for this channel'|i18n( 'design/admin/xmlexport' )}" name="AddExportButton" onclick="javascript:window.location ={concat( 'ezxmlexport/edit/(type)/export/(customer)/', $customer.id)|ezurl( 'single', 'full' )}"/>
            <input class="button" type="button" value="{'Back to channel list'|i18n( 'design/admin/xmlexport' )}" name="BackCustomerListButton"onclick="javascript:window.location ={'ezxmlexport/menu/'|ezurl( 'single', 'full' )}"/>
        </div>
