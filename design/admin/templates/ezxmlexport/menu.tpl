...
<form name="CustomerListForm" action="{'ezxmlexport/delete/'|ezurl( 'no', 'full' )}" method="post">

    <div class="context-attributes">

        <ul>
            <li><a href="{'ezxmlexport/runningexports'|ezurl('no', 'full')}">{'Show running export'|i18n('design/admin/xmlexport')}</a></li>
            <li><a href="{'ezxmlexport/edit/(type)/customer/'|ezurl('no', 'full')}">{'Add an export channel'|i18n('design/admin/xmlexport')}</a></li>
        </ul>

        <h3>{'Export channel list'|i18n('design/admin/xmlexport')}</h3>
        {def $customerList = fetch( 'ezxmlexport', 'customers' )}
        <table class="list" cellspacing="0">
            ...
            {foreach $customerList as $customer sequence array( bglight, bgdark ) as $tr_class}
                ...
                {set $view_link = concat( 'ezxmlexport/view/customer/', $customer.id)|ezurl('no', 'full')}
                {set $edit_link = concat( 'ezxmlexport/edit/(type)/customer/(customer)/', $customer.id)|ezurl('no', 'full')}
                <tr class={$tr_class}>
                    <td>
    ...
    {* DESIGN: Control bar START *}<div class="box-bc"><div class="box-ml"><div class="box-mr"><div class="box-tc"><div class="box-bl"><div class="box-br">
        ...
        <div class="block">
            <input type="hidden" name="RedirectURI" value="ezxmlexport/menu"/>
